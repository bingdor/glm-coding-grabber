...
> 使用自动锁单功能前，需要在 Tampermonkey 设置中 **允许页面重定向**，否则锁单请求会被拦截。
...
#### 4.2 允许页面重定向（自动锁单功能必须）

在 Tampermonkey **设置** 页面中，确保 **允许页面重定向** 为开启状态。自动锁单功能需要发起 create-sign 接口请求，如果禁用重定向会导致锁单失败。
...
4. 如果本地路径不同，修改脚本中的本地文件 `@require` 路径

![修改本地地址](doc/3.改本地地址.png)
...
- 本地验证码服务需要在抢购前启动
- 使用自动锁单需在 Tampermonkey 中 **允许页面重定向**
- 能否抢到仍需运气，建议提前做好准备

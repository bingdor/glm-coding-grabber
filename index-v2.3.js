...
(function (win) {
            ...
            while (true) {
                try {
                    ...
                    if (resp.code === 200 && resp.data && resp.data.productList) {
                        ...
                        log('库存', 'batch-preview 繁忙(555)，2s后重试...');
                        await sleep(2000);
                        continue;
    ...
    function findNextAvailableProduct(currentProduct) {
        if (!currentProduct.soldOut) return currentProduct;
        ...
        for (var i = 0; i < order.length; i++) {
            ...
            if (product && !product.soldOut) return product;
        }
        return null;
    }
    ...
    async function tryPreviewWithRetry(captchaResult, product) {
        ...
        if (resp.code === 200) {
            ...
            log('接口', '已售罄');
            // preview 单个接口确认售罄 → 立即更新 PRODUCTS 中该产品的售罄状态
    ...
    async function executePurchase() {
        ...
        state.wasSoldOutAtStart = product.soldOut;

        // 接受调剂：如果当前套餐已售罄，切换到下一个有货的
        // 但即使全部售罄也不直接放弃，第一次必须尝试 preview
        if (acceptAlternativeEnabled() && product.soldOut) {
            ...
            // 如果全部售罄，仍用当前产品尝试 preview（batch-preview 可能是缓存数据）
        }
        ...
        while (state.running) {
            ...
            if (product.soldOut) {
                if (acceptAlternativeEnabled()) {
                    ...
                    product = nextProduct;
                } else if (!state.wasSoldOutAtStart) {
                    // 不接受调剂且启动时有货，运行中变为售罄 → 终止
                    addSoldOutTokenRow(product.name + ' 已售罄');
                    terminateScript('该套餐已经售罄');
                    return false;
                }
                // 不接受调剂但启动时就已售罄 → 继续（用户明知售罄仍启动）
            }
    ...
    function createControlPanel() {
        ...
        panel.innerHTML =
            ...
            '</div>' +
            '<div style="text-align:right;font-size:10px;color:#999;padding:2px 4px 0 0;">v2.3.0</div>' +
            ...
            '<h3>使用说明 <span style="font-size:11px;color:#888;font-weight:normal;">v2.3.0</span></h3>' +
            ...
            '<p style="color:#e6a23c;font-size:11px;margin-bottom:0;">支持作者 👉 <a href="https://www.bigmodel.cn/glm-coding?ic=XYXVH4BD28" target="_blank" style="color:#409eff;">用邀请链接购买享5%优惠</a></p>' +
            '</div>' +
            ...
            '勾选"接受调剂"后，如果目标套餐已售罄，脚本会按下面的优先级顺序自动切换到下一个有货的套餐。</p>' +
    ...
    function updateProductInfo() {
        ...
        if (el && product) {
            ...
            if (!showSoldOut && product.soldOut) {
                // 抢购开始前，不显示售罄字样，但颜色用橙色提示
                stockText = '';
                el.textContent = product.productId + ' ' + priceText;
